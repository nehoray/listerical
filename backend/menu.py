...
def get_menu_by_date_sq(chosen_date, mydb):
...
def get_opennig_hours_sql(chosen_date, mydb):
    sql_cmd = "SELECT day_part, time(start_time) AS start_time, time(end_time) AS end_time FROM listerical_db.menu where date(start_time) = %s "
    mycursor = mydb.cursor(dictionary=True)
    mycursor.execute(sql_cmd, [chosen_date])
    return parse_result(mycursor)
